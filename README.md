...
这是一个**分层视觉语言操作原型**。第八阶段并非让端到端大模型直接输出完整双球放置轨迹：语言理解采用受约束的任务解析，目标定位来自 RGB 颜色视觉与相机几何，底层执行使用可解释的抓取/放置技能，并在任务完成后以视觉进行复核。仓库保留了颜色抓取行为克隆的示教数据与检查点，用于展示从数据、训练到闭环评估的学习链路。

它不应被描述为真实机器人抓取、开放词表 VLA、真实物理接触泛化或室外部署。更多边界见 [架构与边界](docs/架构与边界.md)。
